...
static dict_table_t* dict_acquire_mdl_shared(
	dict_table_t*	table,
	bool		dict_locked,//FIXME: do not allow dict_locked=true!
...
retry_mdl:
	...
	// FIXME: ut_ad(!sync_check_iterate(sync_check()));
	// (we must not hold any InnoDB mutexes or rw-locks while waiting)

...
/** Increments the modify clock of a frame by 1. The caller must
(1) own the buf_pool->LRU_list mutex and block bufferfix count
    has to be zero,
(2) or own an x-lock on the block.
(3) or the block must belong to intrinsic table.
...
page frame exists, or NULL. The caller must hold either the appropriate hash
lock in any mode, either the LRU list mutex. Note: even though bpage is not
declared a const we don't update its value. It is safe to make this pure.

...
struct purge_node_t{
	...
	/** persistent cursor used in searching the virtual secondary
	index record. */
	btr_pcur_t	sec_pcur;
	/** Heap to store virtual column information. */
	...
	/** Check whether the virtual column information exists
	for the purge node.
	@return true if virtual column exists. */
	bool	is_vcol_info_exist();

	/** Validate the virtual column information for the purge node.
	@return true if mariadb table opened successfully. */
	bool	validate_vcol_info();
};

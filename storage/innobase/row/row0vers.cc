	...
	purge_vcol_info_t*	vcol_info)
{
	...
	if (vcol_info != NULL && vcol_info->mariadb_table == NULL) {
		...
		ut_ad(vcol_info->is_first_fetch());
		return;
	}
	...
	purge_vcol_info_t*	vcol_info)
{
	...
	if (trx_undo_roll_ptr_is_insert(t_roll_ptr)) {
		...
		if (vcol_info != NULL && !vcol_info->is_vcol_uses()) {
			mtr_commit(mtr);
		}
...
# define dbug_v_purge false
			...
			if (trx_undo_roll_ptr_is_insert(t_roll_ptr)
				...
				if (vcol_info != NULL
				    && vcol_info->is_first_fetch()) {
					ut_ad(vcol_info->is_vcol_uses());
					return false;
				}

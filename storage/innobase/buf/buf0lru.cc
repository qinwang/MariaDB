...
and flush list mutexes and do a thread yield. Set the current page
...
rescan:
	...
	for (bpage = UT_LIST_GET_LAST(buf_pool->flush_list);
		...
		if (observer && observer->is_partial_flush()) {
			...
			flush list mutex but not the LRU_list mutex.
...
{
	for (;;) {
		/* TODO: It should be possible to avoid locking the
		LRU list mutex here. */
		mutex_enter(&buf_pool->LRU_list_mutex);
...
@param[in]	scan_all	scan whole LRU list if true, otherwise
				scan only BUF_LRU_SEARCH_SCAN_THRESHOLD
				blocks
	...
	bool		scan_all)
{
	...
	if (use_unzip_list) {
		freed = buf_LRU_free_from_unzip_LRU_list(buf_pool,
							 scan_all);
	}

	if (!freed) {
		freed = buf_LRU_free_from_common_LRU_list(buf_pool,
							  scan_all);	
	}
...
block muteexes will be released.
@param[in,out]	bpage	block, must contain a file page and
			be in a state where it can be freed; there
			may or may not be a hash index to the page
@param[in]	zip	true if should remove also the compressed
			page of an uncompressed page. */
void

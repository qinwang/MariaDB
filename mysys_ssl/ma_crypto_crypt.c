...
#if defined(HAVE_CRYPTO_BCRYPT)
BCRYPT_ALG_HANDLE MA_BCRYPT_ALG_MD5;
BCRYPT_ALG_HANDLE MA_BCRYPT_ALG_SHA1;
BCRYPT_ALG_HANDLE MA_BCRYPT_ALG_SHA256;
BCRYPT_ALG_HANDLE MA_BCRYPT_ALG_SHA384;
BCRYPT_ALG_HANDLE MA_BCRYPT_ALG_SHA512;
BCRYPT_ALG_HANDLE MA_BCRYPT_ALG_AES;
#endif

static int ma_crypto_initialized= 0;

int ma_crypto_init()
{
  if (!ma_crypto_initialized)
  {
#if defined(HAVE_CRYPTO_BCRYPT)
  if (BCryptOpenAlgorithmProvider(&MA_BCRYPT_ALG_AES, BCRYPT_AES_ALGORITHM, NULL, 0) < 0 ||
      BCryptOpenAlgorithmProvider(&MA_BCRYPT_ALG_MD5, BCRYPT_MD5_ALGORITHM, NULL, 0) < 0 ||
      BCryptOpenAlgorithmProvider(&MA_BCRYPT_ALG_SHA1, BCRYPT_SHA1_ALGORITHM, NULL, 0) < 0 ||
      BCryptOpenAlgorithmProvider(&MA_BCRYPT_ALG_SHA256, BCRYPT_SHA256_ALGORITHM, NULL, 0) < 0 ||
      BCryptOpenAlgorithmProvider(&MA_BCRYPT_ALG_SHA384, BCRYPT_SHA384_ALGORITHM, NULL, 0) < 0 ||
      BCryptOpenAlgorithmProvider(&MA_BCRYPT_ALG_SHA512, BCRYPT_SHA512_ALGORITHM, NULL, 0) < 0)
    return 1;
#endif
    ma_crypto_initialized= 1;
  }
  return 0;
}

void ma_crypto_deinit()
{
  if (ma_crypto_initialized)
  {
#if defined(HAVE_CRYPTO_BCRYPT)
    BCryptCloseAlgorithmProvider(MA_BCRYPT_ALG_AES, 0);
    BCryptCloseAlgorithmProvider(MA_BCRYPT_ALG_MD5, 0);
    BCryptCloseAlgorithmProvider(MA_BCRYPT_ALG_SHA1, 0);
    BCryptCloseAlgorithmProvider(MA_BCRYPT_ALG_SHA256, 0);
    BCryptCloseAlgorithmProvider(MA_BCRYPT_ALG_SHA384, 0);
    BCryptCloseAlgorithmProvider(MA_BCRYPT_ALG_SHA512, 0);
#endif
    ma_crypto_initialized= 0;
  }
}
                                    ...
                                    unsigned int iv_len __attribute__((unused)))
{
...
#elif defined(HAVE_CRYPTO_BCRYPT)
  {
	...
	ctx->AlgHdl= MA_BCRYPT_ALG_AES;

    /* create iv object */
    if (iv && iv_len)
    {
      DWORD real_len= iv_len= AES_BLOCK_SIZE;
    ...
    /* load symmetric key */
    status= BCryptGenerateSymmetricKey(ctx->AlgHdl, &ctx->KeyHdl, NULL, 0, (PBYTE)key, key_len, 0);
    if (status < 0)
...
static inline void ctr_increment(unsigned int size, unsigned char *ctr)
{
  do {
    ...
    unsigned int *dlen)
{
  ...
  *dlen= 0;
...
#ifdef HAVE_CRYPTO_BCRYPT
  {
    ...
    PUCHAR pIv= (ctx->mode == MA_AES_ECB) ? 0 : ctx->iv;
    ULONG ivLen= (ctx->mode == MA_AES_ECB) ? 0 : ctx->iv_len;
...
void ma_crypto_crypt_deinit(MA_CRYPTO_CRYPT_CTX cctx)
{
  ...
  if (ctx)
  {
    ...
    ctx->ctx= NULL;
#elif defined(HAVE_CRYPTO_GNUTLS)
    if (ctx->ctx)
      free(ctx->ctx);
    ctx->ctx= NULL;
#elif defined(HAVE_CRYPTO_BCRYPT)

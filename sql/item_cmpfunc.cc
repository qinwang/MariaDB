...
bool Arg_comparator::set_cmp_func(Item_func_or_sum *owner_arg,
                                  Item **a1, Item **a2)
{
  thd= current_thd;
  owner= owner_arg;
  ...
  if (a1[0]->cmp_type() == ROW_RESULT || a2[0]->cmp_type() == ROW_RESULT)
  {
...
void Item_func_between::fix_length_and_dec()
{
  ...
  */
  ...
  DBUG_ASSERT(args[1]->cmp_type() != ROW_RESULT);
...
bool Item_func_between::fix_length_and_dec_traditional()
{
  THD *thd= current_thd;
  if (m_compare_handler.cmp_type() == STRING_RESULT &&
      ...
      !thd->lex->is_ps_or_view_context_analysis())
  {
        ...
        field_item->field_type() ==  MYSQL_TYPE_YEAR)
    {
      ...
      if (cvt_arg1 && cvt_arg2)
        m_compare_handler.set_handler_by_field_type(MYSQL_TYPE_LONGLONG);
    }
  }
  return false;
}
...
longlong Item_func_between::val_int_cmp_temporal()
{
  ...
  ptr= &args[0];
  enum_field_types f_type= field_type_for_temporal_comparison(m_compare_handler.type_handler());
...
void Item_func_in::fix_length_and_dec()
{
  ...
  Item_result tmp_compare_type= STRING_RESULT;
  for (i= 0; i <= (uint)TIME_RESULT; i++)
  {
    if (found_types & (1U << i))
    {
      (type_cnt)++;
      tmp_compare_type= (Item_result) i;
    }
  }

  if (type_cnt == 1)
  {
    if (tmp_compare_type == STRING_RESULT &&
        agg_arg_charsets_for_comparison(cmp_collation, args, arg_count))
      return;
    arg_types_compatible= TRUE;

    if (tmp_compare_type == ROW_RESULT)
    {
  ...
  */
  if (type_cnt == 1 && const_itm && !nulls_in_row())
  {
    ...
    */  
    if (args[0]->real_item()->type() == FIELD_ITEM &&
        !thd->lex->is_view_context_analysis() && tmp_compare_type != INT_RESULT)
    {
      Item_field *field_item= (Item_field*) (args[0]->real_item());
      if (field_item->field_type() ==  MYSQL_TYPE_LONGLONG ||
          field_item->field_type() ==  MYSQL_TYPE_YEAR)
      {
        ...
        if (all_converted)
          tmp_compare_type= INT_RESULT;
      }
    }
    switch (tmp_compare_type) {
    case STRING_RESULT:
  ...
  if (tmp_compare_type != ROW_RESULT)
    m_compare_handler.set_handler_by_result_type(tmp_compare_type);
  max_length= 1;
}

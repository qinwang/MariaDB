...
sp_head *Sp_handler_package_procedure::
sp_find_routine(THD *thd,
                const Database_qualified_name *name,
                bool cache_only) const
{
  Prefix_name_buf pkgname(thd, name->m_name);
  DBUG_ASSERT(pkgname.length);
  return sp_find_package_routine(thd, pkgname, name, cache_only);
}


sp_head *Sp_handler_package_function::
sp_find_routine(THD *thd,
                const Database_qualified_name *name,
                bool cache_only) const
{
...
int Sp_handler_package_procedure::
sp_cache_routine(THD *thd,
                 const Database_qualified_name *name,
                 bool lookup_only, sp_head **sp) const
{
  DBUG_ENTER("Sp_handler_package_procedure::sp_cache_routine");
  Prefix_name_buf pkgname(thd, name->m_name);
  DBUG_ASSERT(pkgname.length);
...
int Sp_handler_package_function::
sp_cache_routine(THD *thd,
                 const Database_qualified_name *name,
                 bool lookup_only, sp_head **sp) const
{
  DBUG_ENTER("Sp_handler_package_function::sp_cache_routine");
  Prefix_name_buf pkgname(thd, name->m_name);
  DBUG_ASSERT(pkgname.length);
  DBUG_RETURN(sp_cache_package_routine(thd, pkgname, name, lookup_only, sp));
}
